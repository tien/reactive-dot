...
const userEventOrig =
  _userEventOrig as unknown as typeof _userEventOrig.default;
...
it("writable count state", async () => {
  ...
  const Counter = () => {
    ...
    return (
      <>
        count: {state}
        <button type="button" onClick={() => dispatch(9)}>
          button
        </button>
      </>
    );
  ...
  it("can recover from error with dependency", async () => {
    ...
    const Counter = () => {
      ...
      return (
        <>
          <div>count: {count}</div>
          <button type="button" onClick={() => setBase((v) => v + 1)}>
            next
          </button>
        </>
      );
  ...
  it("can recover with intermediate atom", async () => {
    ...
    const Counter = () => {
      ...
      return (
        <>
          <div>count: {count}</div>
          <button type="button" onClick={() => refresh((c) => c + 1)}>
            refresh
          </button>
        </>
      );
